...
### 发音、翻译与制作人
Apple 在 WWDC25 上发布的新版 Apple Music 已经支持发音和翻译功能，但 ATT 使用的翻译格式与 Apple 官方格式并不相同
>[!note]
>ATT 的逐字音译格式是正确的，无需修改，但逐行音译仍然并不相同，你可以参考以下翻译的方案进行转换
...
```
#### 背景人声翻译
如果你的歌词中包含 “背景人声”（x-bg），该提示词不能帮你进行转换，你需要手动按照 `主歌词（背景人声）` 的格式进行记录。括号是全角括号。  
如果背景人声在主歌词前面，则应该使用 `（背景人声）主歌词` 的格式

在后续更新中 Apple 支持了使用 `<span>` 标签在翻译中区分背景人声和主歌词，格式如下：
```xml
<text for="L14">
    <span 
        xmlns:ttm="http://www.w3.org/ns/ttml#metadata"
